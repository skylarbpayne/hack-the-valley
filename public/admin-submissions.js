...
function render(submissions) {
  ...
  submissionsEl.innerHTML = submissions.map((submission) => {
    ...
    const uploadHtml = uploads.length ? uploads.map((upload) => `
      <a class="inline-flex items-center gap-2 rounded-lg bg-slate-950 border border-slate-700 px-3 py-2 text-sm text-cyan-300 hover:text-cyan-200" target="_blank" rel="noopener" href="${mediaUrl(upload.key)}">
        <span>${escapeHtml(upload.kind || 'file')}</span>
        <span class="text-slate-500">${escapeHtml(upload.filename || upload.key)}</span>
      </a>
    `).join('') : '<span class="text-slate-500">No uploaded files</span>';
    ...
    return `
      <article class="rounded-2xl bg-slate-900 border border-slate-800 p-6">
        ...
        <div class="mt-4 flex flex-wrap gap-2">${uploadHtml}</div>
      </article>
    `;
  }).join('');
}
