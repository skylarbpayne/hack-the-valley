...
- "Publishing" a post as an email blast goes through `POST /api/blog/broadcast` (admin session + roles required), which reuses the post content to create and schedule a Resend broadcast. The control lives in the "Blog" section of the `/admin` organizer page. Pass `{ dryRun: true }` to preview the rendered email without sending.
- Code structure follows the domain pattern: `functions/_lib/domain/blog-broadcast.js` is the Blog Broadcast domain — it owns the `blog_broadcast_sends` table, the lifecycle, the idempotency rule, and all Resend orchestration (`scheduleBroadcast`, `listRecentSends`, `reconcileBroadcastSends`). The route `functions/api/blog/broadcast.js` stays thin (auth, render the post into email HTML, delegate). Pure email-rendering helpers live in `functions/_shared/blog-broadcast.js`.
